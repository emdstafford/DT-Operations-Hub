...
import { getHistorySupervisors, getMissedStopsHistory, getReportHistory, type MissedStopsSnapshot, type ReportSnapshot } from "@/lib/reportHistory";
...
export default function HistoryPage() {
  ...
  const [correctedSupervisors, setCorrectedSupervisors] = useState<Record<string, SummaryRow[]>>({});
  ...
  useEffect(() => {
    if (!report || correctedSupervisors[report.id]) return;
    let active = true;
    setSupervisorsLoading(true);
    void getHistorySupervisors(report.periodStart, report.periodEnd)
      .then((rows) => {
        if (active) setCorrectedSupervisors((current) => ({ ...current, [report.id]: rows }));
      })
      .catch(() => undefined)
      .finally(() => { if (active) setSupervisorsLoading(false); });
    return () => { active = false; };
  }, [report, correctedSupervisors]);
  const reportSupervisors = report ? correctedSupervisors[report.id] ?? report.supervisors : [];
  ...
  return <main className="page-shell">
    ...
    {report && <div className="report-stack">
      ...
      <div className="two-column">
        <section className="panel overflow-hidden"><div className="panel-heading"><h2>Supervisor performance</h2>{supervisorsLoading && <span>Refreshing dated assignments…</span>}</div><div className="table-scroll"><table className="data-table"><thead><tr><th>Supervisor</th><th>Total</th><th>Completed</th><th>Incomplete</th><th>Completion</th></tr></thead><tbody>{reportSupervisors.map((row, index) => <tr key={row.key} className={index < 5 ? "top-performer-row" : ""}><td className="font-semibold text-navy">{row.label}</td><td>{number(row.totalStops)}</td><td>{number(row.completedStops)}</td><td>{number(row.incompleteStops)}</td><td>{percent(row.percentComplete)}</td></tr>)}</tbody></table></div></section>
    ...
    {missed && <div className="report-stack history-section">
      <div className="section-heading"><p className="eyebrow">Missed-stop reporting</p><h2>Missed Stops History</h2></div>
