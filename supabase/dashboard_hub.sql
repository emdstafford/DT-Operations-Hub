...
begin
  ...
  supervisor_contracts as (
    select supervisor_name supervisor, coalesce(f.contract_number,'Unmapped') contract_number,
      count(*)::bigint load_count, sum(f.total_stops)::bigint total_stops,
      sum(f.completed_stops)::bigint completed_stops, sum(f.incomplete_stops)::bigint incomplete_stops
    from filtered f
    cross join lateral unnest(case when cardinality(f.supervisors)=0
      then array['Unassigned']::text[] else f.supervisors end) supervisor_name
    where coalesce(cardinality(p_supervisors),0)=0 or supervisor_name=any(p_supervisors)
  ...
  select jsonb_build_object(
    ...
    , 'supervisor_contracts', coalesce((select jsonb_agg(to_jsonb(x) || jsonb_build_object('completion_percent',
      case when x.total_stops=0 then 0 else round(x.completed_stops::numeric/x.total_stops,6) end) order by x.supervisor, (case when x.total_stops=0 then 0 else x.completed_stops::numeric/x.total_stops end), x.contract_number) from supervisor_contracts x),'[]'::jsonb)
  ) into result;
  return result;
end;
$$;
