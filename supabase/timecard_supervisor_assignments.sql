-- Run in Supabase SQL Editor after dated_supervisor_assignments.sql.
-- Allows DT report managers to fill missing assignments for the dates in a timecard.
...
begin
  if not public.is_approved(array['admin','uploader']) then
    raise exception 'Report manager access required' using errcode = '42501';
  end if;
