...
export default function FuelMileagePlanner({ start, end, contracts, planOptions, selectedContract, canEdit, onRowsChange }: {
  ...
  return <section className="panel fuel-mileage-panel">
    ...
    <div className="table-scroll"><table className="data-table"><thead><tr><th>Contract</th><th>Planned miles</th><th>Expected gallons</th><th>Purchased gallons</th><th>Difference</th><th>Fuel spend</th><th>Status</th></tr></thead><tbody>{rows.map((row) => <tr key={row.contract} className={row.status === "review" ? "fuel-policy-alert-row" : ""}><td className="font-semibold text-navy">{row.contract}</td><td>{row.coveredDays > 0 ? number(row.plannedMiles,1) : "—"}</td><td>{row.gallonsCoveredDays === row.totalDays && row.totalDays > 0 ? number(row.expectedGallons,1) : "—"}</td><td>{number(row.purchasedGallons,1)}</td><td>{row.status === "incomplete" ? "—" : `${row.variancePercent >= 0 ? "+" : ""}${number(row.variancePercent,1)}%`}</td><td>{currency(row.purchasedFuelCost)}</td><td>{row.status === "incomplete" ? row.coveredDays < row.totalDays ? `Mileage setup needed (${row.coveredDays}/${row.totalDays} days)` : `MPG setup needed (${row.gallonsCoveredDays}/${row.totalDays} days)` : row.status === "review" ? `Needs review (>${number(row.alertAbovePercent,1)}%)` : "Within estimate"}</td></tr>)}</tbody></table></div>
