...
export default function HolidayHoursImport() {
  ...
  const [result, setResult] = useState<HolidayHoursImportResult | null>(null);
  ...
  async function processFile(file: File) {
    ...
    try {
      ...
      setResult(createHolidayHoursImport(await file.text()));
    } catch (caught) {
  ...
  return <div className="payroll-tool-stack">
    <section className="panel payroll-instructions">
      ...
      <div className="calculation-note">
        ...
        <span>Total employee hours ÷ 2 ÷ 40 × 8, rounded to two decimals. The maximum is 8.00 hours.</span>
      </div>
    ...
    {result && <>
      <section className="payroll-summary-grid" aria-label="Import summary">
        ...
        <article><span>Capped at 8.00</span><strong>{result.cappedEmployeeCount.toLocaleString()}</strong></article>
      ...
      <section className="panel payroll-results">
        <div className="panel-heading payroll-result-heading">
          <div><p className="eyebrow">Step 2</p><h2>Review calculated hours</h2><span>{fileName}</span></div>
