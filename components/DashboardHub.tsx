  ...
  return <div className="report-stack">
    ...
    {loading ? <section className="hub-loading">Loading your operations picture…</section> : <>
      ...
      {dashboardView === "investigate" && investigateTab === "contracts" && <><section className="contract-review-bar panel">
        ...
        {!dashboardFuelOnly && <button className="hub-secondary-link" disabled={selectedContracts.length !== 1 || loading || reviewLoading} onClick={() => void printPerformanceReview()}>{reviewLoading ? "Building…" : "Print contract review"}</button>}
      </section>
      ...
      {dashboardView === "overview" && <section className="panel attention-panel dashboard-attention-summary"><div className="panel-heading"><div><p className="eyebrow">USPS performance</p><h2>Contracts below 95%</h2></div><span>{displayDate(start)} – {displayDate(end)}</span></div><div className="attention-list">{data.contracts.filter((row) => Number(row.completion_percent) < 0.95).slice(0,5).map((row,index) => { const content = <><span>{index+1}</span><strong>{row.contract_number}</strong><em>{percent(row.completion_percent)}</em><small>{number(row.incomplete_stops)} incomplete · <b className={`contract-health ${contractHealth(row.completion_percent).className}`}>{contractHealth(row.completion_percent).label}</b></small></>; return dashboardFuelOnly ? <div key={row.contract_number}>{content}</div> : <Link key={row.contract_number} href={`/contracts/${encodeURIComponent(row.contract_number || "Unmapped")}?start=${start}&end=${end}`}>{content}</Link>; })}{!data.contracts.some((row) => Number(row.completion_percent) < 0.95) && <div className="location-empty">All contracts with loads in this period are at or above 95%.</div>}</div>{!dashboardFuelOnly && <Link className="dashboard-summary-link" href={`/contracts?start=${start}&end=${end}`}>Browse all contracts →</Link>}</section>}
      {dashboardView === "overview" && <DashboardFuelChecks start={start} end={end} contracts={data.contracts} />}
      {dashboardView === "overview" && <SupervisorReportOverview supervisors={data.supervisors} supervisorContracts={data.supervisor_contracts} contracts={data.contracts} notes={operationalNotes} start={start} end={end} showRankings={isEntireReport} showContractLinks={!dashboardFuelOnly} />}
