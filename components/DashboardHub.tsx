...
function filteredEmail(data: HubData, start: string, end: string, supervisors: string[], contracts: string[]) {
  ...
  const supervisorDetail = data.supervisors.map((supervisor, supervisorIndex) => {
    const rows = data.supervisor_contracts.filter((row) => row.supervisor === supervisor.supervisor).map((row) => `<tr style="${bottomContracts.has(row.contract_number) ? "background:#f9e8ea" : ""}"><td style="${nameCell}">${escapeHtml(row.contract_number)}</td><td style="${cell}">${number(row.total_stops)}</td><td style="${cell}">${number(row.completed_stops)}</td><td style="${cell}">${number(row.incomplete_stops)}</td><td style="${cell}">${percent(row.completion_percent)}</td></tr>`).join("");
