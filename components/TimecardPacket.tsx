...
const fieldLabels: Record<Field, string> = { last: "Last name", first: "First name", contract: "Worked department / contract", inTime: "In time (includes date)", outTime: "Out time", hours: "Hours", payCode: "Pay code (exclude PTO)" };
...
export default function TimecardPacket() {
  ...
  return <div className="report-stack timecard-stack">
    ...
    <section className="panel timecard-settings no-print">
      ...
      {file && <details className="timecard-column-options" open={!validColumns(file)} key={`${file.name}-${file.sheet}-${file.headerRow}`}><summary>Match file columns {validColumns(file) ? "(review or change)" : "— action needed"}</summary><p>Select the sheet and header row, then match each required field to its column. The file stays in this browser tab.</p><div className="timecard-fields"><label>Sheet<select value={file.sheet} onChange={(event) => setFile((current) => { if (!current) return current; const sheet = event.target.value; const headerRow = 0; const headings = current.sheets[sheet]?.[headerRow] ?? []; return { ...current, sheet, headerRow, columns: detectColumns(headings), employeeIdColumn: headings.findIndex((name) => ["position id", "employee id", "file number", "file #"].includes(normalize(name))) }; })}>{Object.keys(file.sheets).map((sheet) => <option key={sheet} value={sheet}>{sheet}</option>)}</select></label><label>Header row<select value={file.headerRow} onChange={(event) => setFile((current) => { if (!current) return current; const headerRow = Number(event.target.value); const headings = current.sheets[current.sheet][headerRow] ?? []; return { ...current, headerRow, columns: detectColumns(headings), employeeIdColumn: headings.findIndex((name) => ["position id", "employee id", "file number", "file #"].includes(normalize(name))) }; })}>{file.sheets[file.sheet].slice(0, 30).map((row, index) => <option key={index} value={index}>Row {index + 1}: {row.filter(Boolean).slice(0, 3).join(" · ").slice(0, 90) || "(blank)"}</option>)}</select></label>{fields.map((field) => <label key={field}>{fieldLabels[field]}<select value={file.columns[field]} onChange={(event) => setFile((current) => current && ({ ...current, columns: { ...current.columns, [field]: Number(event.target.value) } }))}><option value={-1}>Choose column</option>{(file.sheets[file.sheet][file.headerRow] ?? []).map((heading, index) => <option key={index} value={index}>{heading.trim() || `(unnamed column ${index + 1})`} · {String.fromCharCode(65 + index)}</option>)}</select></label>)}</div>{!validColumns(file) && <p role="alert">Match every field to a different column before the report can be printed or saved.</p>}{validColumns(file) && result.rows.length === 0 && <p role="alert">No timecard rows were found. Check the selected sheet, header row, and In time and Hours columns.</p>}{validColumns(file) && result.invalid > 0 && <p role="alert">{result.invalid} rows could not be read. Check the mapping and data before saving or printing.</p>}</details>}
    ...
    {ready && <div className="timecard-packet"><div className="timecard-screen-heading no-print"><h2>Packet preview</h2><p>Check the hours and contract assignments before printing the packet.</p></div>{printContracts.map(({ contract, people }) => {
      ...
      return <section className="timecard-contract" key={contract}><header><div><h2>Contract {contract}</h2><span>{payrollName.trim() ? `${payrollName.trim()} · ` : ""}{dateLabel(periodStart)} – {dateLabel(periodEnd)}</span></div><div className="timecard-supervisor-heading"><span>{approvedWithoutSupervisor.has(contract.trim().toUpperCase()) && !supervisors.length ? "No supervisor required" : `Supervisor${supervisors.length === 1 ? "" : "s"}`}</span><strong>{supervisors.length ? supervisors.join(", ") : approvedWithoutSupervisor.has(contract.trim().toUpperCase()) ? "Approved" : supervisorStatus === "loading" ? "Loading…" : supervisorStatus === "error" ? "Unavailable" : "Not assigned"}</strong><small>{people.length} employees</small></div></header>
        <div className="timecard-contract-totals"><div className="timecard-contract-hours"><strong>Contract hours</strong><HoursInline current={total(contractRows)} currentName={payrollName.trim() || "this report"} previous={previousPayroll ? { name: previousPayroll.payroll_name, hours: savedHours(previousPayroll, contract) } : null} /></div><div className="timecard-entry-count"><span>Time entries</span><strong>{contractRows.length.toLocaleString()}</strong></div></div>
