...
function HoursInline({ current, first, previous, currentName }: { current: number; first: { name: string; hours: number } | null; previous: { name: string; hours: number } | null; currentName: string }) {
  if (!first && !previous) return null;
  return <span className="timecard-compare-inline">{first && <span>First {first.name}: <b>{hoursLabel(first.hours)}</b></span>}{previous && <span>Previous {previous.name}: <b>{hoursLabel(previous.hours)}</b></span>}<span>Current {currentName}: <b>{hoursLabel(current)}</b></span>{previous && <span className={current < previous.hours ? "timecard-hours-down" : current > previous.hours ? "timecard-hours-up" : ""}>Change: <b>{hoursChange(current, previous.hours)}</b></span>}</span>;
}
...
export default function TimecardPacket() {
  ...
  const unassigned = supervisorStatus === "ready" ? contracts.filter(({ contract, people }) => !approvedWithoutSupervisor.has(contract.trim().toUpperCase()) && !supervisorsFor(contract, people.flatMap((person) => person.rows)).length) : [];
  ...
  return <div className="report-stack timecard-stack">
    ...
    {ready && <div className="timecard-packet"><div className="timecard-screen-heading no-print"><h2>Packet preview</h2><p>Check the hours and contract assignments before printing the packet.</p></div>{contracts.map(({ contract, people }) => {
      ...
      return <section className="timecard-contract" key={contract}><header><div><p>Davenport Transportation · Timecard review</p><h2>Contract {contract}</h2><span>{payrollName.trim() ? `${payrollName.trim()} · ` : ""}{dateLabel(periodStart)} – {dateLabel(periodEnd)}</span></div><div className="timecard-supervisor-heading"><span>{approvedWithoutSupervisor.has(contract.trim().toUpperCase()) && !supervisors.length ? "No supervisor required" : `Supervisor${supervisors.length === 1 ? "" : "s"}`}</span><strong>{supervisors.length ? supervisors.join(", ") : approvedWithoutSupervisor.has(contract.trim().toUpperCase()) ? "Approved" : supervisorStatus === "loading" ? "Loading…" : supervisorStatus === "error" ? "Unavailable" : "Not assigned"}</strong><small>{people.length} employees</small></div></header>
        <div className="timecard-contract-totals"><div className="timecard-contract-hours"><span>Contract hours</span><strong>{hoursLabel(total(contractRows))}</strong><HoursInline current={total(contractRows)} currentName={payrollName.trim() || "this report"} first={firstPayroll ? { name: firstPayroll.payroll_name, hours: savedHours(firstPayroll, contract) } : null} previous={previousPayroll ? { name: previousPayroll.payroll_name, hours: savedHours(previousPayroll, contract) } : null} /></div><div><span>Employees</span><strong>{people.length}</strong></div><div><span>Time entries</span><strong>{contractRows.length.toLocaleString()}</strong></div></div>
