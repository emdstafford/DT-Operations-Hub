...
const validColumns = (source: Source) => {
  ...
  return fields.every((field) => source.columns[field] >= 0 && source.columns[field] < width) && new Set(Object.values(source.columns)).size === fields.length && (source.dateColumn < 0 || source.dateColumn < width);
};
...
export default function TimecardPacket() {
  ...
  async function readFile(upload?: File) {
    ...
    try {
      ...
      setFile({ name: upload.name, sheets, sheet: best.sheet, headerRow: best.headerRow, columns: detectColumns(headings), dateColumn: detectDate(headings), employeeIdColumn: detectEmployeeId(headings) });
