...
export default function TimecardPacket() {
  ...
  return <div className="report-stack timecard-stack">
    ...
    <section className="panel timecard-settings no-print">
      ...
      <div className="timecard-payroll-fields"><label>Payroll name<input type="text" value={payrollName} maxLength={100} placeholder="Example: September 30 payroll" onChange={(event) => setPayrollName(event.target.value)} /></label><label>Pay date<input type="date" value={payDate} onChange={(event) => setPayDate(event.target.value)} /></label></div>
    ...
    {ready && payrollName.trim() && payDate ? <TimecardComparisons entries={historyEntries} start={periodStart} end={periodEnd} sourceName={file?.name || "Timecard report"} payrollName={payrollName.trim()} payDate={payDate} /> : ready && <section className="panel no-print timecard-comparisons"><h2>Save and compare payrolls</h2><p>Enter the payroll name and pay date above to save the hour totals and compare this report with past payrolls. You can still print the timecards now.</p></section>}
